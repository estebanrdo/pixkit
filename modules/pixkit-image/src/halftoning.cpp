...
bool pixkit::halftoning::ordereddithering::KackerAllebach1998(const cv::Mat &src1b, cv::Mat &dst1b){
	...
	for(int i=0; i<src1b.rows; i+=sizeDitherArray){
		for(int j=0; j<src1b.cols; j+=sizeDitherArray){
			...
			while(doneflag != true){	// \A4\A3\A5i\BBP\A4\A7\ABe\AA\BADA label\ACۦP
				...
				for(int m=-OSCW;m<=OSCW;m++){
					for(int n=-OSCL;n<=OSCL;n++){
						if(i+m>=0&&i+m<src.rows&&j+n>=0&&j+n<src.cols){	// \A6b\BCv\B9\B3\BDd\B3\F2\A4\BA
							if(CM[(i+m)%ClassMatrixSize][(j+n)%ClassMatrixSize]>number){		// \A5i\A5H\C2X\B4\B2\AA\BA\B0ϰ\EC
								fm+=DW[m+OSCW][n+OSCL];
							}
						}
					}
...
bool pixkit::halftoning::dotdiffusion::LippensPhilips2007(const cv::Mat &src, cv::Mat &dst){
	...
	float	hysteresis=0.;	// \A4\B4\A6b\B4M\A7\E4\B0\DD\C3D, \A6\B9\AC\B0\ABD0\A4\A7\B5\B2\AAG\A4\A3\A6P\A9\F3\B8ӽפ\E5, \ACG\A5\FD\A6\E6\B3]\A9w\AC\B00
				...
				if(entireCM[i][j]==number){
					...
					for(int m=-OSCW;m<=OSCW;m++){
						for(int n=-OSCW;n<=OSCW;n++){
							if(i+m>=0 && i+m<src.rows && j+n>=0 && j+n<src.cols){	// \A6b\BCv\B9\B3\BDd\B3\F2\A4\BA
								if(entireCM[i+m][j+n]<number){		// \BDd\B3\F2\A4\BA\C2X\B4\B2\B9L\AA\BA\B0ϰ\EC
					...
					if(b_fm!=0){
						for(int m=-OSCW;m<=OSCW;m++){
							for(int n=-OSCW;n<=OSCW;n++){
								if(i+m>=0 && i+m<src.rows && j+n>=0 && j+n<src.cols){	// \A6b\BCv\B9\B3\BDd\B3\F2\A4\BA
									if(entireCM[i+m][j+n]<number){		// \BDd\B3\F2\A4\BA\C2X\B4\B2\B9L\AA\BA\B0ϰ\EC
										comp += coe[m+OSCW][n+OSCW] / b_fm * (RegImage[i+m][j+n] < 128 ? -1 : 1);	// \ABݽT\BB{
									}
								}
							}
						}
					...
					// \A8\FA\B1oerror, \A8è\FA\B1ohalftone \BF\E9\A5X.
					double	error;
					...
					for(int m=-OSCW;m<=OSCW;m++){
						for(int n=-OSCW;n<=OSCW;n++){
							if(i+m>=0&&i+m<src.rows&&j+n>=0&&j+n<src.cols){	// \A6b\BCv\B9\B3\BDd\B3\F2\A4\BA
								if(entireCM[i+m][j+n]>number){		// \A5i\A5H\C2X\B4\B2\AA\BA\B0ϰ\EC								
